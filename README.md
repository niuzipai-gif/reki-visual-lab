...
Reki Visual Annotation Lab 是一个面向 coser、摄影修图和视觉讲解场景的图片标注工作台。它支持在浏览器本地导入图片、添加红色信号标记、调整框选区域、管理图层、导出透明效果层、GIF、短视频和实况素材包，并提供不上传原图的隐私友好流程。

## 日本語紹介

Reki Visual Annotation Lab は、コスプレ写真、レタッチ指示、ビジュアル解説向けの画像アノテーション作業台です。ブラウザ上で画像を読み込み、赤いシグナルマーカー、自由なリサイズ、レイヤー管理、透明エフェクトレイヤー、GIF、短い動画、Live Photo 用素材パックの書き出しに対応します。元画像をサーバーへ送らない、プライバシーを重視したワークフローです。
