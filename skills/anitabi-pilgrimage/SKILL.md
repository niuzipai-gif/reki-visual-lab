...
## 三语说明 / 3言語の説明 / Trilingual description

### 中文

用于查询动漫圣地巡礼点、现实取景地、附近巡礼点、地图链接、集数时间、截图和地址反查。支持 Bangumi/Anitabi 实时查询；要求区分已返回数据、无数据和网络失败，不得编造地点、坐标、集数或图片。

### 日本語

アニメの聖地巡礼・実在のロケ地・周辺の巡礼スポット・地図リンク・話数と時刻・スクリーンショット・住所からの逆引きを調べるためのスキルです。Bangumi/Anitabi のリアルタイム情報を使い、取得済み・データなし・通信失敗を分け、場所・座標・話数・画像を推測で補いません。

固定配信順：場所名 → Google マップリンク → 対応画像。次の場所へ進む前に、現在の場所を一件ずつ完了します。

### English

Use this skill to find anime pilgrimage locations, real-world filming spots, nearby landmarks, map links, episode timestamps, screenshots, and reverse lookups from addresses. Query live Bangumi/Anitabi data and keep returned data, no-data cases, and network failures separate; never invent a place, coordinate, episode, or image.

Fixed delivery order: place name → Google Maps link → matching image. Complete one point before moving to the next.

For every language, follow the fixed point-by-point delivery contract below: send each point’s name, then its Google map link, then its matching image before moving to the next point.
