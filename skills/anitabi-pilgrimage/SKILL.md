...
4. For a screenshot, use `--img N`. Deliver the resulting local file through the host agent’s supported image/file channel; do not call a hard-coded messenger or vision-provider command.
...
## Character-image review

`scan_characters.py` is provider-neutral. It creates a manifest with `has_char: null` for unreviewed images and can merge a labels JSON file written by the host agent’s own vision capability:
...
Accepted labels are booleans only. Preserve `null` when the image is missing or uncertain; never convert “looks like a character” into a verified fact without visual evidence.
