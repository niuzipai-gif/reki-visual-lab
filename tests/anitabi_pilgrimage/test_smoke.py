...
class AnitabiSkillSmokeTests(unittest.TestCase):
    ...
    def test_trilingual_explanation_and_homepage_entry(self):
        skill_text = (PACKAGE / "SKILL.md").read_text(encoding="utf-8")
        readme_text = (REPO / "README.md").read_text(encoding="utf-8")
        for marker in ("中文", "日本語", "English"):
            self.assertIn(marker, skill_text)
            self.assertIn(marker, readme_text)
        self.assertIn("Google 地图链接", skill_text)
        self.assertIn("Google マップリンク", skill_text)
        self.assertIn("Google Maps link", skill_text)
        self.assertIn("skills/anitabi-pilgrimage/SKILL.md", readme_text)
